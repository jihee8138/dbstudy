-- 1. 대소문자 변환하기
SELECT UPPER('apple') -- APPLE (대문자로 변환)
     , LOWER ('APPLE') -- applea (소문자로 변환)
     , INITCAP('APPLE') -- Apple (첫글자만 대문자로 변환)
  FROM DUAL;
...
-- 3. 문자 연결하기
    ...
    -- 2) CONCAT(A, B) : 연결 함수 (최대 2개까지만 연결 가능)
SELECT 'A' || 'B' || 'C'
     , CONCAT(CONCAT('A', 'B'), 'C')
  FROM DUAL;
...
SELECT CONCAT(CONCAT(SUBSTR(NAME, 1, 1), '*'), SUBSTR(NAME, 3))
  FROM EMPLOYEE_T;
