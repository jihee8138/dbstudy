...
/*
    ...
    3. ANSI 문법
        1) 왼쪽 외부 조인
        SELECT 조회할 칼럼, ...
          FROM 테이블1 LEFT [OUTER] JOIN 테이블2 --'OUTER'는 생략 가능
            ON 조인조건
        2) 오른쪽 외부 조인
        SELECT 조회할 칼럼, ...
...
SELECT E.EMPLOYEE_ID, E.FIRST_NAME, J.JOB_TITLE, J.MAX_SALARY, E.SALARY
...
-- 5.부서별 평균급여를 조회하시오. 부서명, 평균급여를 조회하시오. 근무 중인 사원이 없으면 평균급여를 0으로 조회하시오.
...
-- 6. 사원번호, 사원명(E), 부서번호, 부서명(D), 지역번호, 지역명(L)을 조회하시오.
...
-- 관계 (1명의 매니저가 N명의 사원을 관리한다.)
-- 1                               N
-- 매니저테이블 : EMPLOYEES M      사원테이블 : EMPLOYEES E
-- PK           : EMLPOYEE_ID      FK         : MANAGER_ID
-- 조인조건     : 
...
-- 9. 같은 부서 내에서 나보다 급여를 더 많이 받는 사원을 조회하시오.
-- 관계
-- 나는 여러 사원과 관계를 맺는다.
