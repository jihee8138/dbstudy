/*
    ...
    5. 형식
          SELECT 조회할 칼럼, 조회할 칼럼, ...
            FROM 테이블명
           WHERE 조건식 (WHERE절부터는 생략 가능)
        GROUP BY 그룹화할 칼럼, 그룹화할 칼럼, ... (GROUP BY절과 HAVING절은 함께 들어가고 함께 빠져야 한다.)
          HAVING 그룹조건식
...
SELECT * -- 모든 칼럼을 의미하는 *, 실제 개발할 때는 사용 금지(조회 성능이 떨어져 전체적인 시스템이 느려지기 때문)
    FROM DEPARTMENT_T;
...
-- 7. 사원 테이블에서 기본급이 2,000,000 ~ 3,000,000 인 사원을 조회하시오.
SELECT *
  FROM EMPLOYEE_T
